...
#   1. Starts Doubao with a loopback-only DevTools port (127.0.0.1, whitelisted range)
#   2. Injects a wallpaper layer + transparency CSS into doubao-chat pages over CDP
#   3. Watches the wallpaper folder - changing the wallpaper applies live (no restart)
#   4. Exits automatically when Doubao exits
#
# Wallpaper folder: %USERPROFILE%\.doubao\wallpaper\
...
$ExeCandidates = @(
    'F:\Program files\Doubao\Doubao.exe',
    'C:\Program files\Doubao\Doubao.exe',
    (Join-Path $env:LOCALAPPDATA 'Doubao\Doubao.exe'))
$AllowedPorts = @(19222, 19223, 19224, 19225, 19226)
...
$ZHBody   = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('5qOA5rWL5Yiw6LGG5YyF5q2j5Zyo6L+Q6KGM77yM5L2G5pyq5Yqg6L295aOB57q444CCCgrlhbPpl63lubbph43lkK/osYbljIXku6XliqDovb3lo4HnurjvvJ8='))
...
$ZHClear  = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('KOW3sua4heepuu+8jOWbnuWIsOaegeWFieWFnOW6lSk='))
...
$ZHConn   = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('5bey6L+e5o6l6LGG5YyF6LCD6K+V56uv5Y+jIA=='))
$ZHNoExe  = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('5pyq5om+5Yiw6LGG5YyF5a6J6KOF55uu5b2V77yIRG91YmFvLmV4Ze+8ie+8jOmAgOWHug=='))
...
# ---------- CSS + bootstrap script injected into the Doubao page ----------
$LAYER_CSS = @'
html,body{background:transparent!important;}
#chat-route-layout,#chat-route-main,#flow_chat_sidebar,#chat-route-main>main{background:transparent!important;}
:root,body,#chat-route-layout{
--color-bg-body:rgba(250,250,252,.62)!important;--dbx-bg-body-web:rgba(250,250,252,.62)!important;
--dbx-bg-body-white:rgba(255,255,255,.66)!important;--chat-bg-color:rgba(255,255,255,.55)!important;
--chatarea-bg-color:rgba(243,245,251,.4)!important;--s-color-bg-body:rgba(255,255,255,.6)!important;
--base-bg-color:rgba(245,245,245,.55)!important;--dbx-bg-base-web:rgba(249,249,249,.6)!important;
--dbx-bg-float:rgba(255,255,255,.92)!important;--bg-input:rgba(255,255,255,.72)!important;
--input-guidance-input-container-background:rgba(255,255,255,.7)!important;}
html[data-theme="dark"],html[data-theme="dark"] body,html[data-theme="dark"] #chat-route-layout{
--color-bg-body:rgba(18,20,24,.6)!important;--dbx-bg-body-web:rgba(18,20,24,.6)!important;
--dbx-bg-body-white:rgba(28,30,34,.6)!important;--chat-bg-color:rgba(22,24,28,.55)!important;
--chatarea-bg-color:rgba(28,30,34,.45)!important;--s-color-bg-body:rgba(20,22,26,.6)!important;
--base-bg-color:rgba(24,26,30,.55)!important;--dbx-bg-base-web:rgba(24,26,30,.6)!important;
--dbx-bg-float:rgba(38,40,46,.92)!important;--bg-input:rgba(38,40,46,.7)!important;
--input-guidance-input-container-background:rgba(38,40,46,.75)!important;}
#chat-route-main>div:first-child,div[class*="h-header-height"]{background-color:rgba(255,255,255,.55)!important;}
html[data-theme="dark"] #chat-route-main>div:first-child,html[data-theme="dark"] div[class*="h-header-height"]{background-color:rgba(22,24,28,.55)!important;}
[class*="guidance-input-surface"]{background-color:rgba(255,255,255,.72)!important;}
html[data-theme="dark"] [class*="guidance-input-surface"]{background-color:rgba(38,40,46,.75)!important;}
#dbwp-wallpaper{position:fixed;inset:0;z-index:-1;overflow:hidden;pointer-events:none;
...
#dbwp-wallpaper img,#dbwp-wallpaper video{width:100%;height:100%;object-fit:cover;display:block;}
#dbwp-wallpaper .dbwp-shade{position:absolute;inset:0;}
#dbwp-wallpaper.aurora::before,#dbwp-wallpaper.aurora::after{
content:"";position:absolute;width:90vmax;height:90vmax;border-radius:50%;will-change:transform;}
#dbwp-wallpaper.aurora::before{
background:radial-gradient(circle at 50% 50%,rgba(59,130,246,.6) 0%,rgba(29,78,216,.32) 32%,transparent 68%);
top:-30%;left:-20%;animation:dbwp-a 26s ease-in-out infinite alternate;}
#dbwp-wallpaper.aurora::after{
background:radial-gradient(circle at 50% 50%,rgba(168,85,247,.55) 0%,rgba(109,40,217,.3) 34%,transparent 68%);
bottom:-35%;right:-25%;animation:dbwp-b 32s ease-in-out infinite alternate;}
@keyframes dbwp-a{0%{transform:translate(0,0) scale(1)}50%{transform:translate(12vw,8vh) scale(1.15) rotate(20deg)}100%{transform:translate(4vw,16vh) scale(.95) rotate(-10deg)}}
@keyframes dbwp-b{0%{transform:translate(0,0) scale(1)}50%{transform:translate(-10vw,-10vh) scale(1.2)}100%{transform:translate(-4vw,-4vh) scale(.9)}}
@media (prefers-reduced-motion: reduce){#dbwp-wallpaper.aurora::before,#dbwp-wallpaper.aurora::after{animation:none;}}
'@
...
$BOOTSTRAP_TEMPLATE = @'
(function(){
if(!window.__dbwpApply){
window.__dbwpApply=function(url,isVideo,opacity,shade){
var l=document.getElementById("dbwp-wallpaper");
...
el.src=url;
l.appendChild(el);
var sh=document.createElement("div");sh.className="dbwp-shade";sh.style.background="rgba(0,0,0,"+shade+")";l.appendChild(sh);
return"applied";};}
var s=document.getElementById("dbwp-style");
if(!s){s=document.createElement("style");s.id="dbwp-style";document.documentElement.appendChild(s);}
s.textContent="__DBWP_CSS__";
var c=document.getElementById("dbwp-custom-style");
if(!c){c=document.createElement("style");c.id="dbwp-custom-style";document.documentElement.appendChild(c);}
c.textContent="__DBWP_CUSTOM_CSS__";
var l=document.getElementById("dbwp-wallpaper");
if(!l){l=document.createElement("div");l.id="dbwp-wallpaper";(document.body||document.documentElement).appendChild(l);
...
function Send-CdpMessage([string]$wsUrl, [string]$method, [string]$paramsJson, [int]$timeoutSec) {
    ...
    try {
        ...
        # 必须丢弃返回值，用状态轮询限时等待（实测 2026-09-14）
...
# 壁纸经本机回环 HTTP 流式供给页面（视频可 Range 拖动），取代旧的 2MB 分块 base64 注入：
# 不再把 47MB 文件读进内存转 ~63MB 字符串、页面侧不再做 O(n²) 拼接，注入从 ~9 秒降到亚秒。
...
function Send-Wallpaper([string]$wsUrl, $wp, [string]$customCss) {
    ...
    $boot = $BOOTSTRAP_TEMPLATE.Replace('__DBWP_CSS__', (Esc-Json $LAYER_CSS)).Replace('__DBWP_CUSTOM_CSS__', (Esc-Json $customCss))
    $r1 = Send-CdpEval $wsUrl $boot 60
    if ($r1 -ne 'ready') { return 'bootstrap-failed: ' + $r1 }
    # 页面 CSP 一般不放行 http:// 媒体源 → 经 CDP 关掉本页 CSP（旧的 data: URL 方案不需要这步）
    # 注意参数名是 enabled（bypass 会被 CDP 以 Invalid parameters 拒掉，CSP 实际没被绕过）
    ...
    $r2 = Send-CdpEval $wsUrl ('window.__dbwpApply("' + $url + '",' + $boolLit + ',' + $op + ',' + $Cfg.darkOverlay + ')') 30
    if ($r2 -ne 'applied') { return 'apply-failed: ' + $r2 }
    # 1.5 秒后问一次媒体健康度；加载失败会在日志可见，下一轮 sig 未变不会自动重试（可删 marker 触发）
    $probe = 'new Promise(function(res){setTimeout(function(){var l=document.getElementById("dbwp-wallpaper");if(!l){res("no-layer");return}var m=l.querySelector("video,img");if(!m){res("no-media");return}if(m.tagName==="VIDEO"){res(m.error?("video-error:"+m.error.code):(m.readyState>=1?"ok":"loading"))}else{res(m.complete?"ok":"loading")}},1500)})'
    $health = Send-CdpEval $wsUrl $probe 15 -AwaitPromise
    return ('apply: ' + $r2 + ' (' + $wp.ext + ', http stream, health=' + $health + ')')
}

# ---------- 0. single instance ----------
$script:Mutex = New-Object System.Threading.Mutex($false, 'Local\ai-wallpaper-doubao')
if (-not $script:Mutex.WaitOne(0)) { Log 'another launcher instance already running, exit'; exit 0 }

# ---------- 1. resolve Doubao / debug port ----------
if (-not (Test-Path $WallDir)) { New-Item -ItemType Directory -Path $WallDir -Force | Out-Null }
...
$doubaoUp = $false
try { $doubaoUp = ($null -ne (Get-Process -Name Doubao -ErrorAction SilentlyContinue)) } catch {}

$port = 0
if ($doubaoUp) {
    foreach ($p in $AllowedPorts) { if (Test-DebugPort $p) { $port = $p; break } }
    if ($port -eq 0) {
        Log 'Doubao running without debug port - asking to restart'
...
if ($port -eq 0) {
    ...
    Log ('starting Doubao with debug port ' + $port + ': ' + $exe)
    Start-Process -FilePath $exe -ArgumentList ("-remote-debugging-port={0}" -f $port) | Out-Null
    $ready = $false
    for ($i = 0; $i -lt 40; $i++) {
...
while ($true) {
    ...
    # wallpaper / custom.css / 轮换槽位变化？-> 本轮强制推送到所有聊天窗口
    ...
    foreach ($t in $targets) {
        ...
        # 只注入主聊天窗口；排除 cross-site-support 等辅助页（对它们发送会挂起）
        if ($t.url -notmatch 'doubao-chat/chat') { continue }
        if (-not $changed) {
            $guard = Send-CdpEval $t.webSocketDebuggerUrl '!!document.getElementById("dbwp-wallpaper")' 8
        ...
        try {
            if ($wp) { Log (Send-Wallpaper $t.webSocketDebuggerUrl $wp $customCss) }
                ...
                $null = Send-CdpEval $t.webSocketDebuggerUrl 'var l=document.getElementById("dbwp-wallpaper");l.innerHTML="";l.className="aurora";"aurora-on"' 30
            }
